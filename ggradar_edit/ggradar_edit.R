...
#' @param axis.label.color text color of axis label #kw
...
ggradar <- function(plot.data,
                    ...
                    axis.label.color = "black", #added_kw
                    axis.lebel.face = "plain", #added_kw
                    grid.min = 0, # 10,
  ...
  base <- ggplot(axis$label) +
    ...
    geom_text(
      ...
      aes(x = x, y = y, label = text), size = axis.label.size, hjust = 1, family = font.radar, color = axis.label.color, fontface = axis.lebel.face
    ) +
  ...
  base <- base + geom_text(
    ...
    aes(x = x, y = y, label = text), size = axis.label.size, hjust = 0.5, family = font.radar, color = axis.label.color, fontface = axis.lebel.face
  )
  # + axis labels for any vertical axes [x>x.centre.range]
  base <- base + geom_text(
    data = subset(axis$label, axis$label$x > x.centre.range),
    aes(x = x, y = y, label = text), size = axis.label.size, hjust = 0, family = font.radar, color = axis.label.color, fontface = axis.lebel.face
  )
